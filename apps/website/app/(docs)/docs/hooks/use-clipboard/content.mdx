---
title: "useClipboard"
description: "Copy text with status lifecycle, accessibility announcements, and optional legacy execCommand fallback."
section: "hooks"
package: "hooks"
status: "stable"
order: 6
surface: "hook"
seoTitle: "useClipboard | Kuzenbo Docs"
seoDescription: "Use useClipboard to copy values with explicit status/error state, ARIA announcements, and optional legacy fallback behavior."
ogImage: "/docs-og/hooks/use-clipboard"
ogImageAlt: "useClipboard documentation"
aliases:
  - "/docs/hooks/browser/use-clipboard"
---
...
- optional legacy fallback (`document.execCommand("copy")`)
...
- `useLegacyFallback?: boolean` (default `false`)

It returns:

- `status: "idle" | "copying" | "copied" | "failed"`
- `errorCode: "clipboard-unavailable" | "permission-denied" | "write-failed" | "fallback-failed" | null`
- `announcement: string`
...
| Scenario                                 | `useLegacyFallback: false`                                                                           | `useLegacyFallback: true`                                                         |
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `navigator.clipboard.writeText` succeeds | returns `ok: true`, status `copied`                                                                  | same                                                                              |
| Clipboard API throws                     | status `failed`, mapped error code (`permission-denied`, `clipboard-unavailable`, or `write-failed`) | attempts `execCommand("copy")`; success => `copied`, failure => `fallback-failed` |
| Clipboard API missing                    | status `failed`, `clipboard-unavailable`                                                             | attempts `execCommand("copy")`; success => `copied`, failure => `fallback-failed` |

The hook auto-resets `copied`/`failed` states after the configured duration.

## SSR and RSC notes

- Client-only hook: it depends on `navigator` and optional DOM fallback APIs.
