...
- Use semantic z-index utilities (`z-overlay`, `z-sticky`, `z-toast-viewport`) instead of one-off numeric values.
- Configure global layer order through theme CSS variables (`--kb-z-*`) when app shells need custom stacking.
