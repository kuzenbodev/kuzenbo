...
order: 14
surface: "utility"
seoTitle: "Theme Runtime | Kuzenbo Docs"
seoDescription: "Understand cookie and storage reconciliation, bootstrap script behavior, and runtime theming APIs."
ogImage: "/docs-og/foundations/theme-runtime"
ogImageAlt: "Theme Runtime documentation"
---
...
2. local storage (`kuzenbo-theme`), then legacy storage key (`theme`)
...
- Legacy storage key support is built in (`LEGACY_THEME_STORAGE_KEY = "theme"`).
- If older apps stored theme under `theme`, bootstrap can still recover and resync to `kuzenbo-theme`.
