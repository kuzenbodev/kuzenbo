...
const MonthLevelGroup = (allProps: MonthLevelGroupProps) => {
  const {
    className,
    __onDayClick,
    __onDayMouseEnter,
  ...
  const monthDisplayContextValue = withWeekNumbers
    ? MONTH_DISPLAY_CONTEXT_VALUES.withWeekNumbers
    : MONTH_DISPLAY_CONTEXT_VALUES.noWeekNumbers;
  return (
    <MonthDisplayContext.Provider value={monthDisplayContextValue}>
      <div
        className={cn(monthLevelGroupVariants(), className)}
        data-slot="month-level-group"
        data-week-numbers={withWeekNumbers || undefined}
        style={{
          ...
          return (
            <MonthLevel
              __getDayRef={(rowIndex, cellIndex, node) => {
                ...
                if (
                  !Array.isArray(controlsRef.current[columnIndex][rowIndex])
                ) {
