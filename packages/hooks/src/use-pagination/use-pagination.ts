...
/**
 * Builds pagination state and helpers for controlled or uncontrolled page
 * navigation.
 *
 * @param {UsePaginationOptions} options - Pagination configuration.
 * @param {number} options.total - Total number of pages.
 * @param {number} [options.page] - Controlled active page.
 * @param {number} [options.initialPage] - Initial active page for uncontrolled
 * usage.
 * @param {number} [options.startValue] - First page number, defaults to `1`.
 * @param {number} [options.siblings] - Number of sibling pages shown around
 * the active page.
 * @param {number} [options.boundaries] - Number of page items always shown
 * at each edge.
 * @param {(page: number) => void} [options.onChange] - Called whenever the
 * active page changes.
 */
export const usePagination = ({
  ...
  const paginationRange = useMemo((): (number | "dots")[] => {
    ...
    const leftSiblingIndex = Math.max(
      activePage - siblings,
      _startValue + boundaries - 1
    );
    const rightSiblingIndex = Math.min(
      activePage + siblings,
      _endValue - boundaries
    );
...
export type UsePaginationOptionsType = UsePaginationOptions;
export type UsePaginationReturnType = UsePaginationReturnValue;
