...
/**
 * Manages horizontal scrolling state and interactions for a scrollable container.
 * Provides directional scroll helpers, edge availability flags, and optional drag-to-scroll handlers.
 *
 * @param {UseScrollerOptions} options - Scroller behavior configuration.
 * @param {number | undefined} options.scrollAmount - Pixel distance used by `scrollStart` and `scrollEnd`.
 * @param {boolean | undefined} options.draggable - Enables mouse drag scrolling when `true`.
 * @param {((state: UseScrollerScrollState) => void) | undefined} options.onScrollStateChange - Called whenever `canScrollStart` or `canScrollEnd` changes.
 */
export const useScroller = <T extends HTMLElement = HTMLDivElement>(
...
export type UseScrollerOptionsType = UseScrollerOptions;
export type UseScrollerReturnType<T extends HTMLElement = HTMLDivElement> =
  UseScrollerReturnValue<T>;
export type UseScrollerScrollStateType = UseScrollerScrollState;
