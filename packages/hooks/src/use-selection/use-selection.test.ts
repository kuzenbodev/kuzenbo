...
describe("@kuzenbo/hooks/use-selection", () => {
  it("correctly returns initial state for an empty data array", () => {
    const data: number[] = [];
    const { result } = renderHook(() => useSelection({ data }));

    expect(result.current[0]).toStrictEqual([]);
    expect(result.current[1].isAllSelected()).toBeFalsy();
    expect(result.current[1].isSomeSelected()).toBeFalsy();
  });
  ...
  it("correctly initializes with defaultSelection", () => {
    ...
    const { result } = renderHook(() =>
      useSelection({ data, defaultSelection })
    );

    expect(result.current[0]).toStrictEqual(
      expect.arrayContaining(defaultSelection)
    );
    expect(result.current[0]).toHaveLength(2);
    expect(result.current[1].isAllSelected()).toBeFalsy();
    expect(result.current[1].isSomeSelected()).toBeTruthy();
  });
  ...
  it("correctly deselects an item", () => {
    ...
    expect(result.current[0]).toStrictEqual(expect.arrayContaining([1, 2]));
    expect(result.current[0]).toHaveLength(2);
  ...
  it("isAllSelected is true when all items are selected", () => {
    ...
    expect(result.current[1].isAllSelected()).toBeTruthy();
    expect(result.current[1].isSomeSelected()).toBeTruthy();
  });
  ...
  it("allows direct setting of selection via setSelection", () => {
    ...
    expect(result.current[0]).toStrictEqual(expect.arrayContaining([10, 30]));
    expect(result.current[0]).toHaveLength(2);
    expect(result.current[1].isAllSelected()).toBeFalsy();
    expect(result.current[1].isSomeSelected()).toBeTruthy();
