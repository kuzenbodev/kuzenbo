...
/**
 * Manages an array in React state and exposes immutable helpers for common list operations
 * like append, prepend, insert, remove, reorder, swap, and targeted item updates.
 *
 * @param {T[] | (() => T[])} [initialValue] Initial list value or lazy initializer function (defaults to an empty array).
 */
export const useListState = <T>(
  ...
  const setItemProp = <K extends keyof T, U extends T[K]>(
    ...
    setState((current) => {
      ...
      cloned[index] = { ...currentItem, [prop]: value } as T;
      return cloned;
    });
...
export type UseListStateReturn<T> = UseListStateReturnValue<T>;
export type UseListStateHandlersType<T> = UseListStateHandlers<T>;
