...
const TEST_STATE: [TestStateItem, TestStateItem, TestStateItem] = [
...
const filterNonBill = (item: TestStateItem) => item.name !== "Bill";

describe("@kuzenbo/hooks/use-list-state", () => {
  ...
  it("filter items with handlers.filter", () => {
    ...
    act(() => {
      const [, handlers] = hook.result.current;
      handlers.filter(filterNonBill);
    });

    const [state] = hook.result.current;
    expect(state).toStrictEqual(TEST_STATE.filter(filterNonBill));
  });
});
