...
/**
 * Returns screen orientation angle and type and keeps them in sync with
 * orientation changes.
 *
 * @param {UseOrientationOptions} [options] - Configuration for initial
 * fallback values and SSR-safe initialization behavior.
 * @param {number} [options.defaultAngle] - Angle used before browser
 * orientation data is available.
 * @param {OrientationType} [options.defaultType] -
 * Orientation type used before browser orientation data is available.
 * @param {boolean} [options.getInitialValueInEffect] - If `true`, resolve
 * the initial value in an effect for SSR safety.
 */
...
export type UseOrientationOptionsType = UseOrientationOptions;
export type UseOrientationReturnValue = UseOrientationReturnType;
